...
% Redefine footontes to appear in the margin (except when inside tcolorbox env)
\makeatletter
\def\ifenv#1{
   \def\@tempa{#1}%
   \ifx\@tempa\@currenvir
...
%\renewcommand\footnote[1]{\ifenv{tcolorbox}{\oldfootnote{#1}}{\refstepcounter{mgncount}\marginpar{{\textsuperscript{\themgncount}}\footnotesize{\emph{#1}}}\footnotemark~}}
\renewcommand\footnote[1]{\refstepcounter{mgncount}\marginpar{{\textsuperscript{\themgncount}}\footnotesize{\emph{#1}}}\footnotemark~}
\NewEnvCode{tcolorbox}{\let\footnote\oldfootnote}

% Redefine quote envirnoment so that's boxed:
%\let\origquote\quote
%\let\endorigquote\endquote

\renewenvironment{quote}{\begin{tcolorbox}\RunEnvCode}{\end{tcolorbox}}
