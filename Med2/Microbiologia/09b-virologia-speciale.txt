\clearpage
\part{Virologia speciale}

# Virus influenzali e parainfluenzali (\TODO{sistemare sezioni})
- __Ribovirus a genoma negativo__ \marginnote{I virus a genoma negativo hanno RNA che non è in grado di funzionare da messaggero. È necessario una RNA polimerasi RNA-dipendente per poter trascrivere il genoma e produrre il trascritto su cui attivare la sintesi proteica}
    - Orthomixovirus (virus influenzali)
    - Paramixovirus (virus parainfluenzali)

## Virus influenzali
- \TODO{guardare per bene tutto, riorganizzare le varie info}

### Anatomia
- __Genoma ad RNA diviso in 7/8 segmenti monocatenari__^[7 segmenti in virus influenzale C; 8 segmenti in virus influenzale A e B]
- Involucro lipidico dalla forma sferica, con incastonate diverse classi di glicoproteine \marginnote{Le proteine di membrana sono quelle maggiormente soggette a mutamenti} \marginnote{Nella denominazione del ceppo influenzale, la denominazione \texttt{HxNy} si riferisce al fatto che quel ceppo ha emoagglutinina X e neuroamminidasi y}
    - __`Neuroamminidasi`__
    - __`Emoagglutinina`__
    - __Protiena M2__ --- permette l'adesione del virus alla cellula target
    - __Proteine `PB1`, `PB2`, `PBA`__ --- \TODO{}
    - __Proteina `NS`__ (non strutturali) --- necessarie per proteggere il genoma una volta esportato, in maniera da evitare che, nella cellula host, sia soggetto a splicing
- Proteine interne
    - __Proteina di matrice M1__ --- \TODO{} no


#### Proteine di membrana
- __Emoagglutinina__
    - È l'antirecettore che lega il virione all'_acido sialico_ presente sulla parete cellulare dell'host
    - 20 tipi differenti
- __Neuroamminidasi__
    - Impedisce che il virione venga neutralizzato dal legame con acido sialico che è contenuto nelle glicoproteine mucose
    - Libera il virione dalla membrana cellulare quando si forma, per staccarlo dalla cellula oramai morente e permettere la diffusione dell'infezione
    - 11 tipologie differenti

## Sviluppi di nuovi ceppi influenzali
- \TODO{}

### Antigenic shift
- \TODO{}

<!-- \normalbox{Le grandi influenze della storia}{ -->
<!-- - __Spagnola__ (H1N1) --- 1918 \TODO{} -->
<!-- - __Asiatica__ (H2N2) --- 1957 \TODO{} -->
<!-- - __Pandemia A__ (H1N1) --- 2009 \TODO{} -->
<!-- } -->

<!-- Tue 28 Apr 2020 02:14:08 PM CEST -->

### Manifestazione clinica
- Incubazione 18-72 ore post inalazione respiratoria, con esordio acuto
- Febbre <38.5°C, più elevata nel bambino
- Tosse
- Mialgia
- Possibili manifestazioni GI (sopratutto adulto)
- Si risolve in 7gg circa, con persistenza di tosse anche per dopo

### Immunità e vaccinazione
- Immunità non altamente protettive
    - Si sviluppa principalmente tramite IgA (non IgG o IgM), che non sono super efficaci
    - I virus mutano molto rapidamente (_antigenic shift_)
- Vaccino rimane mezzo più efficace per prevenzione. Una buona fetta di popolazione purtroppo non risponde al vaccino (10-30% di mancata copertura)
    - \goldstandard Vaccino inattivato mediante formalina
    - Vaccino con soli antigeni (H ed N)
- Tendenzialmente la vaccinazione è per
    - H1N1 + H3N2 + Ceppo A + Ceppo B
    - Ceppi di isolamento recente/rilevanti nell'anno passato
- Importante vaccinare/proporre vaccinazione a categorie a rischio o chi convive con categorie a rischio
    - Anziani (+ conviventi)
    - Donatori/operatori sanitari
    - Immunocompromessi, a seconda del tipo di vaccino (+ conviventi)
    - Neonati (+ conviventi)

### Diagnosi
- Diagnosi: clinica (particolare attenzione a pz. immunocompromessi o di esito dubbio, dove una diagnosi precisa è _fondamentale_) ⇒ __PRC multiplex con pannello respiratorio__ (batteri + virus)
- Metodiche
    - Ricerca diretta di antigeni su tampone naso/faringe, aspirati n/f, lavaggio BA (BAL)
    - \goldstandard PCR multiplex (pannello respiratorio)

# Paramyxoviridae
- Famiglia di ribovirus a genoma con polarità negativa che contiene, tra gli altri
    - Paramyxovirus --- parainfluenzale umano 1 e 3
    - Rubulavirus --- parainfluenzale umano 2, 4a, 4b, parotite
    - Morbillovirus --- morbillo
    - Pneumovirus --- virus respiratorio sinciziale umano

## Anatomia generale
- Il genoma __non è segmentato__
    - Monocatenario
    - Polarità negativa
    - RNA
    - Complessato con __proteine NP__ (p. _del nucleocapside_)
- Nel peplos si trova il __complesso trascrittasico__, composto da 2 famiglie proteiche
    - __Proteina L__
    - __Proteina P__
- All'interno del virone si trova anche la __proteina V__ (_fattore di virulenza_)
    - Impedisce la trascrizione di geni dell'host che codificano per IFNα e IFNβ
- Membrana lipidica di origine cellulare, a due facce
    - Una faccia interna, costruita da __proteina M__
    - Una faccia esterna
- Nella membrana ci sono le proteine di spike
    - __Proteina F__ (di fusione) --- stimola la fusione del corpo del virione con la parete cellulare della cellula host. Tipica e caratteristica della famiglia dei Paramixovirus
    - __Glicoproteina HN__ --- a funzione composta
        - Neuroamminidasica
        - Emoagglutinasica

## Ciclo replicativo
0. Legame con cellula target grazie a glicoproteina HN -- che si lega con acido sialico
1. Fusione (grazie a proteina F) \TODO{la proteina f ha ruolo importante: capire quale}
2. \TODO{da sbobbe}

## Manifestazione clinica
- Breve periodo di incubazione (4-6gg)
- L'infezione in soggetti competenti è solitamente limitata all'epitelio delle vie respiratorie superiori (mucosa nasofaringea in adulto, territorio più esteso in pz. ped)
    - 25% dei casi infezione prosegue verso laringe/trachea/bronchi (⇒ laringo-tracheo-bronchiti) + edema laringeo
    - In rarissimi casi prosegue verso le vie profonde con bronchite/bronchiolite/polmonite
- Ruolo importante, nel determinare la gravità della risposta, dal sistema immunitario dell'host
    - ↑ livelli di citochine infiammatorie e chemochine ⇒ risposta pesante ⇒ sintomatologia pesante
- Clearance virale: 4-5 settimane

## Diagnosi
- Interesse in pz. altrimenti sano: scarso
    - In adulto solitamente non viene fatta
    - In pz. ped diventa rilevante, perché può avere complicazioni (discesa nelle vie profonde) che vanno considerate
- Come
    - Ricerca antigeni in secrezioni nasofaringee
    - PCR con pannello respiratorio
    - NO: coltivazione, sierologia

## Rubulavirus (parotite)
- Uomo è unico serbatoio naturale

### Clinica
- Incubazione lunga
- Eliminazione del virus anche in contesto asintomatico
- Febbre non elevata
- Stanchezza, anoressia, perdita appetito
- Dolore in zona collo, e dietro alle orecchie \asidefigure{img/parotite.png}{}
- Ingrossamento anche molto evidente delle parotidi

#### Complicazioni
- Orchite^[Non frequente in maschi prepuberi, frequente in maschi post-puberi]/ooforite^[In femmine puberi]  che raramente esita in sterilità
- Encefalite e/o meningite asettica (10/15% dei casi). Possibile anche se infezione parotidea rimane subclinica (paucisintomatica) e, quindi, misconosciuta
- Sordità temporanea/permanente

### Patogenesi
0. Infezione tramite goccioline di saliva
1. Colonizzazione del tratto respiratorio superiore e dei linfonodi regionale
2. \TODO{}

### Diagnosi
- Clinica (se c'è, a volte la manifestazione è subclinica) + PCR su saliva (specialmente se sg. paucisintomatico)
- PCR tampone
- PCR su urine
- Test indiretti per IgM, abbastanza accurati

## Morbillovirus

### Epidemiologia
- 1333 casi di morbillo nel 2017, di cui l'88% in sg. non vaccinati
    - Distribuiti omogeneamente tra regioni del nord e del sud
- Outbreak recenti \TODO{}
- Gruppi di età \TODO{}

### Patogenesi
- \TODO{}

### Presentazione clinica
- Incubazione: 10-15 gg
- Prodromi (4 gg max)
    - Febbre alta, tosse, mal di gola, congiuntivite
    - Se prodromi proseguono ⇒ verosimile forma complicata
- Esantema maculopapuloso caratteristico \asidefigure{img/esantema-morbillo.png}{}
- Macchie di Koplik \TODO{figura} nella mucosa buccale
- Disseminazione sistemica
    - Polmonite
    - Meningite e/o encefalite
- Contagiosità, riferita all'evento centrale della patologia (esantema)
    - Da 5 gg prima dell'esantema
    - A 6-21 gg dopo l'esantema, con viremia che si abbassa costantemente da dopo esantema

#### Complicazioni
- 1/4 necessario ricovero
- Otite media ($\leadsto$ sordità)
- Encefalite in fase post-acuta (0.5/1%)
- Encefalite post-infettiva ritardata (probabilmente per sovrapposizione di azione auto-immune)
- \pat{pess} (Panencefalite subacuta sclerosante a placche)
- Episodi di auto-immunità

##### \pat{PESS}
- Risultato di una rara (~ 1:300K infetti) infezione cronica. Malattia neurodegenerativa in cui il virus si riproduce nel distretto encefalico
- Segni
    - Anticorpi liquorali (BEE alterata, non deve succedere)
    - ↑↑ ig sieriche
    - Nucleocapsidi del paramyxovirus in neuroni e cellule gliali + ag morbillo^[In realtà non proprio morbillo, ma leggermente differente, quasi come se si fosse "modificato" nel tessuto cerebrale, per questo viene indicato come __virus PESS__] (⇐ biopsia)
- 3 fasi
    - Fase prodromica: disturbi attenzione, umore, comportamento
    - Fase di stato: deterioramento vigilanza e coscienza + 
    - \TODO{terza fase}
- Genoma virale: __mutato__, con interessamento di
    - Proteina F
    - Emoagglutinina
    - Proteina M

### Diagnosi
- Sierologici possibili da 3 giorni _prima_ dell'esantema

<!-- Thu 30 Apr 2020 09:26:22 AM CEST -->
# Togavirus
- \TODO{}

## Rosolia
- \TODO{}

### Rosolia in gravidanza
- \TODO{fino a Thu 30 Apr 2020 09:31:03 AM CEST }
- Naturalmente diventa _imperativo_ capire lo stato immunologico _pre-gravidanza_
    - IgG specifiche ⇒ immunità
    - No IgG specifiche ⇒ suscettibilità

#### Diagnosi di rosolia congenita
1. Determinazione dello stato immunologico pre gravidanza della madre
2. Se la madre è suscettibile
    - Diagnosi pre-natale _con obiettivo di trattamento_ \TODO{?}
        - Prelievo di sangue fetale --- Titolo IgM + isolamento virale + PCR
        - Amniocentesi --- per isolamento virale + PCR
    - Diagnosi post-natale
        - Urina/Tampone n/f --- isolamento virale + PCR
        - Sangue --- Titolo IgG IgM + PCR
        - Monitorizzazione continua ogni 3/6 mesi

### Epidemiologia
- 100K bambini/anno nascono con CRS
    - 50% bambini non hnno accesso al vaccino per rosolia

# Papillomavirus (HPV)
- Virus relato definitivamente allo sviluppo di forme cancerose
    - \pat{cercvico-carcinoma}
    - Cancro pene, ano vulva
    - Tumori testa/collo
- Infezione principalmente tramite __contatto diretto__, ma non si possono escludere anche contagi per via indiretta^[Il virus è stato ritrovato anche in bocca o nel letto ungueale]
    - Ricerca in corso, ma non sembra possibile trasmissione verticale
    - Ricerca in corso, ma non sembra che il virus diffonda per via ematica \TODO{accenno a fine lezione, controllare parte terminale sbobina}

\greenbox{Altri virus oncogeni}{
\begin{longtable}[]{@{}ll@{}}
\toprule
\begin{minipage}[b]{0.28\columnwidth}\raggedright
Virus\strut
\end{minipage} & \begin{minipage}[b]{0.66\columnwidth}\raggedright
Tumore provocato\strut
\end{minipage}\tabularnewline
\midrule
\endhead
\begin{minipage}[t]{0.28\columnwidth}\raggedright
HPV\strut
\end{minipage} & \begin{minipage}[t]{0.66\columnwidth}\raggedright
Carcinoma della cervice uterina, ano, vulva,vagina,pene, orofaringe,
cute\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.28\columnwidth}\raggedright
HCV e HBV\strut
\end{minipage} & \begin{minipage}[t]{0.66\columnwidth}\raggedright
Tumori epatici, linfoma non Hodgkin\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.28\columnwidth}\raggedright
HTLV1\strut
\end{minipage} & \begin{minipage}[t]{0.66\columnwidth}\raggedright
Leucemie e linfoma a cellule T\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.28\columnwidth}\raggedright
HHV8 (Herpesvirus)\strut
\end{minipage} & \begin{minipage}[t]{0.66\columnwidth}\raggedright
Sarcoma di Kaposi\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.28\columnwidth}\raggedright
EBV\strut
\end{minipage} & \begin{minipage}[t]{0.66\columnwidth}\raggedright
Tumori n/f, linfoma di Hogkin, linfomi dello stomaco\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.28\columnwidth}\raggedright
MCV (Merkel Cell Polyomavirus)\strut
\end{minipage} & \begin{minipage}[t]{0.66\columnwidth}\raggedright
Tumore cellule di Merkel\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

Naturalmente anche batteri (\emph{Helicobacter} in primis) sono relati con lo sviluppo di patologie oncologiche (cancro dello stomaco)
}

## Tassonomia
- α papillomavirus (mucosi)
    - Condilomi/carcinomi ano-genitali
    - Papillomi/carcinomi orali
- β, γ, μ, ν papillomavirus (cutanei)
    - Verruche e carcinomi cutanei

## Anatomia
- \TODO{}
- Genoma: 8K bp \asidefigure{img/genoma-papillomavirus.png}{}
    - Regione LCR (Long Control Region)
        - Origine replicativa
        - Elementi di controllo per processo trascrizionale
    - Regione E (Early)
        - \TODO{}
        - Oncoproteina E6
        - Oncoproteina E7
        - \TODO{Maledetto microsoft teams di merda, non è ripartita la chiamata dopo la pausa}
    - Regione L (Late)
        - Proteina L1: proteina capsidica esterna
        - Proteina L2: proteina capsidica interna

## Contagio ed infezione
0. __Contatto diretto su citotipi cellulari__ appropriati
    - Legame: proteine capsidiche con `eparan-solfato`
1. Penetra nelle cellule, approfondandosi per tutto l'epitelio, infettando gli epiteliociti. A seconda dello strato dell'epitelio il virus si comporta in modo differente
    - Nelle cellule dello strato basale: mantenimento del genoma e replicazione lenta
    - Nello startato soprabasale/spinoso: amplificazione del genoma
    - Nello strato granulare: assemblamento di nuovi virioni
    - Nello strato squamoso: le cellule morte non costituiscono una barriera efficiente, e quindi si osserva il rilascio del virus

## Carcinogenesi

### Storia naturale dell'infezione
1. Infezione
2. Prosecuzione dell'infezione
    - Infezione transitoria (80%)
        - \TODO{}
    - Infezione persistente (20%)
        - \TODO{}

### Cofattori nella carcinogenesi
- Deficit immunitari
- Non uso di profilattici \marginnote{L'uso del profilattico purtroppo non esclude la possibilità di infezione, perché se il virus si trova nella mucosa di uno dei due partner c'è una concreta possibilità che questo si depositi sulla parte esterna, aumentando poi il rischio di contagio nel momento in cui il profilattico viene maneggiato}
- Uso a lungo termine di contraccettivi orali
- Multiparità
- HIV^[Si registra una elevata presenza di HPV in soggetti HIV+]
    - \TODO{}
    - Effetto additivo nel processo di progressione neoplasica

<!-- Mon 04 May 2020 09:15:56 AM CEST -->

- \TODO{primi dieci minuti}

![](img/grading-carcinoma-hpv.png)\ 


## Epidemiologia
- \TODO{}

## Prevenzione e screening

### Test
- PAP test (Test di Papanicolau -- 1946)
    - Screening citologico adeguatamente sensibile (80%), ma non troppo specifico (60%) \marginnote{Livelli ottimi di sensibilità o specificità si attestano attorno al 98\%}
    - Consiste nel prelievo di materiale citologico per la ricerca di lesioni pre-cancerose
        - Riduzione della quantità di citoplasma
        - Aumento nel numero di cellule
- HPV test
    - Diagnosi basata su PCR, con l'obiettivo di identificare i ceppi di papillomi eventualmente presenti
    - Elevata sensibilità, ma bassa specifici

### Modello di prevenzione
- Modello classico
    1. PAP test
    2. Colposcopia
    3. Biopsia mirata
- Modello recente
    1. Test molecolare (sensibile, ma non specifico)
    2. PAP test (specifico, ma non sensibile)
    3. Colposcopia
    4. Biopsia
    5. Valutazione del rischio e individuazione della strategia appropriata
        - Sorveglianza
        - Terapia
- Ad oggi in regione la scelta di come procedere dipende dall'età
    - 25 > età > 29 --- PAP test ogni 3 anni
    - 30 > età > 64 --- HPV test ogni 5 anni
    - Centro Screening AUSL bologna disponibile con numero verde per dare informazioni

\begin{figure}
\centering
Tra i 25 e i 29 anni\\
\includegraphics{img/pap-test-modelllo-screening.png}\\
\rule{0.5\linewidth}{\linethickness}\\
Tra i 30 e i 64 anni\\
\includegraphics{img/hpv-screening-dopo-30aa.png}
\caption{Algoritmo di screening per l'HPV a seconda dell'età}
\end{figure}

\yellowbox{HPV nel maschio}{
- L'infezione nel maschio
    - Ha probabilità di infezione più alta nell'arco di tutta la vita
    - Clearance più veloce (6 mesi)
- No programma di screening al momento (inattuabili \TODO{perché})
}

## Altre tipologie di tumori indotti da HPV

### Cancro anale
\TODO{}

### Tumori testa-collo
- 5% del totale delle neoplasie
- Sopravvivenza a 5 anni: 50%
- 95% da HPV16
- Concause che portano ad \att oncogeni e \ini oncosoppressori

## Vaccinazione preventiva
- Necessario che preceda l'esposizione, ovvero prima dell'inizio dei rapporti sessuali
- Inducono alti titoli di IgG (sia nelle secrezioni cervicali che sieriche)
    - Vaccino a proteine purificate (in questo caso, proteina `L1` di superficie)
- \TODO{mi sono perso i diversi tipi}
- Vaccinazione in adulti
    - Solo se HPV-DNA\ - , altrimenti non ha senso
    - \TODO{}

# Hepatovirus
- Molti virus sono responsabili di epatite, ma quelli responsabili di _epatiti primarie_ sono
    - HAV (RNA)
    - HBV (DNA)
    - HCV (RNA)
    - HEV (DNA)
    - Virus delta (RNA)
    - \TODO{meccanismi di trasmissione}

## HAV
- Virus a RNA della famiglia dei _Picornaviridae_
- ssRNA+
- \TODO{anatomia esterna}
- Trasmissione oro-fecale
    1. Introduzione attraverso cibo
    2. Prima replicazione a livello dell'intestino
    3. Seconda replicazione a livello del fegato
    4. Trasmissione fecale (anche 7 gg prima della comparsa dei sintomi)

### Epidemiologia
- \TODO{}

- \TODO{}

## HBV
- Famiglia: _Orthoepadnavirus_
- Trasmissione
    - Via ematica ($\star$)
    - Via sessuale
    - Altri liquidi biologici (essudati da ferita, saliva...)
    - Trasmissione verticale

### Genoma
- 4 sequenze codificatrici
    1. Core (C)
    2. Polimerasi 90KD (P)
    3. Superficie (S)
    4. Proteina transattivante (X)
        - \ini oncosoppressori (p53 e altri)
- \TODO{$\star$ HBcAg, HBeAg, HBsAg, e tutti gli altri antigeni. Vedere sbobina}

### Epidemiologia
- 8 diversi genotipi (identificati con le lettere A-H)
- In EU i principali genotipi sono A, D e G
- In ↓ da anni '90 grazie a introduzione della vaccinazione obbligatoria
- \TODO{}

<!-- Tue 05 May 2020 02:30:30 PM CEST -->
### Patogenesi e presentazione clinica
- \TODO{}

#### Storia clinica
- Infezione \asidefigure{img/storia-clinica-eptite.png}{}
    - 90% --- infezione acuta silente
    - 10% --- epatite acuta
        - 0.001%: epatite fulminante
- 10%: infezione cronica
    - 0.04% portatori asintomatici
    - 0.06% epatite cronica (50% severa e 50% moderata)
        - 50% cirrosi
            - 10% carcinoma epatocellulare

#### Marcatori e manifestazione clinica
- \TODO{}

#### Infezione occulta da HBV
- Infezione da HBV a bassissima carica virale (0.2 copie/ml sangue)
    - Livelli di HbsAg talmente bassi che non sono evidenziabili con tecniche commerciali
    - Possibile riattivazione
    - Evento veramente raro
- \TODO{}
- \TODO{forme e-minus}

#### HBV e carcinoma del fegato
- Se infezione persistente: rischio x200 di sviluppare carcinoma primitivo del fegato rispetto ai non infetti
- 2 meccanismi
    1. Ruolo diretto di HBV \TODO{}
    2. Ruolo indiretto di HBV \TODO{}

### Vaccinazione e prevenzione
- Vaccino a DNA ricombinante in lievito, con __proteine codificate dal gene S__ (in modo che venga espresso HBsAg)
    - No effetti collaterali
    - Ottima risposta: copertura + nel 95% dei casi
    - 3 dosi
        1. Tempo 0
        2. +1 mese
        3. +5/6 mesi da 2/a dose
- Obbligatoria nel 3/o mese di vita. Unica eccezione: vaccinazione + Ig anti-HBV preformati alla nascita (entro 12 h) se madre è HBV+ (presenta HBsAg \TODO{se solo HBsAg non può voler dire che madre è solo vaccinata? Perché?})
- Vaccino consigliato per personale sanitario a rischio

## HCV
- \TODO{}
- Trsmissione
    - Sangue infetto (principale)
    - Via sessuale
    - Verticale

\greenbox{Trasmissione verticale di HCV}{
\begin{itemize}
\tightlist
\item
  Se no cofattori o comorbidità importanti, la gravidanza non
  costituisce rischio di aggravamento di una potenziale patologia
  epatica
\item
  Occorre considerare la possibilità di trasmissione verticale. Rischi:

  \begin{itemize}
  \tightlist
  \item
    Viremia materna (PCR + ⇒ 1 nato infetto/20)
  \item
    Tossicodipendenza materna attiva o pregressa (sembra dovuto dal
    fatto che lfc materni sono spesso infettati e funzionano da
    serbatoio)
  \item
    Coinfezione con HIV ⇒ 1 nato infetto/10

    \begin{itemize}
    \tightlist
    \item
      Immunodepressione
    \item
      Spesso tossicodipendenza
    \end{itemize}
  \end{itemize}
\end{itemize}
}

### Anatomia
- Anatomia del virus \TODO{}
- Genoma \TODO{}
- \TODO{Ehm... un pochino tanto}

### Storia clinica
- Infezione \asidefigure{img/flowchart-storia-epatite-c.png}{}
    - \TODO{}

### Terapia
- Strategia: inibitori `NS5`
    - $\star$ \far{sofosbuvir}: sostituto nucleotidico inibitore di NS5 (che, essendo una RNA polimerasi RNA-dipendente, se viene inibita blocca la replicazione virale). Attivo contro tutti i genotipi virali (farmaco _pan-genotipico_)
        - Farmacoresistenze: stanno cominciando ad emergere, anche se ancora in forma lieve. Rischio concreto di trasmettere varianti pre-farmacoresistenti
    - \far{simeprevir}: attivo con genotipi 1 e 4, \TODO{}

## HDV

## HEV

# Herpesvirus

## Herpes simplex

<!-- Thu 07 May 2020 10:03:30 AM CEST -->
<!-- TODO manca tutta la prima ora di oggi -->


### Presentazione e storia clinica

0### Ricorrenza
- Herpes si ripresenta (riattivazione virale) se si ha un calo delle difese immunitarie che prima controllavano senza problemi la replicazione virale e avevano costretto il virus in fase latente
    - Stress/emozioni forti
    - Traumi
    - Febbre
    - Esposizione alla luce solare

#### Manifestazione clinica

##### Herpes simplex di tipo 1
- Raramente diffusione alla congiuntiva (cheratocongiuntivite)
    - Auto-inoculazione è meccanismo principale
    - Possibili lesioni o ulcerazioni

##### Herpes simplex di tipo 2
- Febbre
- Dolore
- Encefalite erpetica
    - Esordio acuto
    - Liquor con pleiocitosi^[Alto numero di cellule]

##### Presentazione neonatale
- Infezione durante il parto ⇒ consigliato parto cesareo per prevenire trasmissione dell'infezione
- Prognosi grama
- No CMI (\TODO{}) efficace
- \TODO{}

| Tipo infezione | Probabilità di trasmissione \TODO{verticale?}|
|---|---|
| Primo episodio a termine di gravidanza | 50% |
| Primo episodio non primario \TODO{what?} | 30% |
|Ricorrente | ~ 0% |

### Terapia
- Bloccanti della DNA polimerasi (\far{aciclovir}, \far{famciclovir})
    - No terapia che permette di rimuovere il virus latente
- Misure di prevenzione generica e buona igiene
    - Evitare il contatto diretto con le lesioni erpetiche
    - Buon lavaggio delle mani

## Varicella Zoster Virus

### Storia clinica
0. Infezione
1. Latenza \TODO{dove}
2. \TODO{}
3. Latentizzazione ?
4. Possibile riattivazione ⇒ \pat{herpes zoster} (_fuoco di S. Antonio_)

### Presentazione
- Rash cutaneo caratteristico
    - Si presenta in 2 asi
        1. A livello del tronco, in espansione
        2. A livello del viso
    - Non toccarli! Esiti cicatriziali
    - Vanno eliminati spontaneamente
- Manifestazione secondaria
    - Nevralgie molto dolorose (30% pz. > 65 aa che sviluppano infezione)
    - Prevenibile con vaccino, che viene eseguito anche secondariamente ad infezione

### Epidemiologia
- Trasmissione per via respiratoria
- 10-20% pz. può sviluppare recidiva in età anziana (_zoster_)

### Terapia
- Nel bambino: no terapia
- Nell'adulto
    - Immunocompentente: no terapia
    - Immunocompromesso o pz. in recidiva
        - Antivirali
        - \far{vzig}: ig specificihe vs VZV
        - Anestetici/analgesici locali
        - Vaccino

#### Vaccinazione
- Importante per prevenire manifestazioni secondarie (_zoster_)
- Offerto a pz. > 65 aa con infezione da varicella precedente
- Vaccino a _virus vivo attenuato_

# Epstein Barr Virus
- \TODO{}
- Trasmissione
    - Via orale (saliva)
    - Documentati casi di trasmissione ematica o attraverso trapianti

<!-- TODO: lezione Fri 08 May 2020 11:00:24 AM CEST -->
