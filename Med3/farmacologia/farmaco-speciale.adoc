...
* Spettro d'azione ampio, non solo TBC
** icon:star[] _M tuberculosisi_, _M leprae_
** Cocchi e Gram^pos^ (anche MRSA)
** _Nesseria meningitidis_ (`rifampicina` usata per PEP in singola dose one-shot)
** _Legionella pneumophila_

NOTE: Morg con gene *rpo^mut^* sviluppano molto velocemente resistenza, per questo si deve impostare tp. in associazione

.Farmacocinetica
* (D) Ottima diffusione tissutale (compreso SNC, osso, occhio); *compresa penetrazione intracellulare*
