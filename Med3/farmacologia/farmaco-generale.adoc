...
* Per ogni fase farmacocinetica si individuano grandezze collegate
	...
	** D → *volume di distribuzione* [__D__] --- volume _teorico_ [l o kg] nel quale il farmaco si distribuisce a concentrazione omogenea
	** M → nr
...
* Farmaci acidi o basici in soluzione acquosa si dissociano in una specie neutra e una specie protonata o deprotonata, in funzione del pH della soluzione in cui si trovano (eq di Henderson--Hasselback) image:img/pka-ph-pkb.png[]
...
* Via intramuscolare ("IM")
	** Rilascio lento e prolungato nel tempo, con assorbimento rapido
	** Maggiormente dolorosa
* Via sottocutanea ("SC")
	** Assorbimento generalmente rapido, con possibilità di regolarlo modificando la costrizione del plesso venoso sottocutaneo (scaldando la pelle, raffreddandola, frizionaldola, usando vasocostrittori...). Facile accesso e somministrazione relativamente sicura
	** C'è comunque rischio nel somministrare
* Via endovenosa ("EV")
	** Assorbimento immediato, biodisponibilità completa e facilmente controllabile. Ottima come via d'emergenza, per infusioni prolungate o grandi volumi
	** Grande rischio di setticemia se non si mantiene la via il più possibile pulita e sterile. Altre complicanze: embolie, tromboflebiti...
* Via endocavitaria (intratecale, intraoculare...)
* Via inalatoria (endobronchiale/polmonare)
...
=== Distribuzione
** *Una volta nel versante plasmatico il farmaco si lega ai _carrier plasmatici_*, con un legame che è [.underline]#in equilibrio# rispetto alla quota che invece è libera. *Da qui, si distribuisce a tutti i distretti corporei*
	** La quota legata è farmacologicamente inattiva, e il rapporto tra quota libera e legata ai carrier è costante e nota per ogni p.a (se albuminemia fisiologica)
